...
datasets=(ycb_t-grasp_n-1000_o-m3_s-2173311271.json ycb_t-move_n-1000_o-m3_s-2703277985.json ycb_t-put-on_n-1000_o-m3_s-1517382128.json ycb_t-put-in_n-1000_o-m3_s-4282974133.json)
...
for data in "${datasets[@]}"; do
  for model in "${models[@]}"; do
    ...
    output_dir=~/VLATest/results/${data%.json}/${model}_2024/
    ...
    if [ ! -d "$output_dir" ] || [ "$(find "$output_dir" -type d | wc -l)" -lt 1000 ] || [ "$(find "$output_dir" -name 'log.json' | wc -l)" -ne 1000 ]; then
      while true; do
        ...
        timeout "${timeout_duration}" bash -c "PYTHONPATH=~/VLATest python3 run_fuzzer.py -s 2024 -m '${model}' -d ../data/'${data}' -io /data/"
