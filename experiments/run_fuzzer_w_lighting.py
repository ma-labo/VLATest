...
if __name__ == '__main__':
    ...
    lighting_random_seed = args.seed if args.seed else np.random.randint(0, 4294967295)  # max uint32
    lighting_fuzzer = RandomLighting(None, lighting_random_seed, step_range=(1, 2), factor_range=(1, 20))

    if args.output:
        result_dir = args.output + f"random_lighting_{lighting_random_seed}"
    else:
        result_dir = str(PACKAGE_DIR) + "/../results/" + f"random_lighting_{lighting_random_seed}"

    os.makedirs(result_dir, exist_ok=True)

    if args.image_output:
        image_dir = args.image_output + f"random_lighting_{lighting_random_seed}"
            ...
            for idx in range(tasks["num"]):
                if idx in valid_tasks:
                    for sample in range(args.samples):
                        lighting[dataset_name][model_name][f"{idx}_{sample}"] = lighting_fuzzer.generate_options()
                ...
                if model == args.model:
                    ...
                    if "grasp" in dataset_name:
                        if 'ycb' in dataset_name:
                            vla = VLAInterface(model_name=model, task="google_robot_pick_customizable_ycb")
                        else:
                            vla = VLAInterface(model_name=model, task="google_robot_pick_customizable")
                    elif "move" in dataset_name:
                        if 'ycb' in dataset_name:
                            vla = VLAInterface(model_name=model, task="google_robot_move_near_customizable_ycb")
                        else:
                            vla = VLAInterface(model_name=model, task="google_robot_move_near_customizable")
                    elif "put-on" in dataset_name:
                        vla = VLAInterface(model_name=model, task="widowx_put_on_customizable")
                    elif "put-in" in dataset_name:
                        vla = VLAInterface(model_name=model, task="widowx_put_in_customizable")
                    else:
                    ...
                    for idx in tqdm(valid_tasks):
                        for sample in range(args.samples):
                            lighting_options = lighting[dataset_name][model_name][f"{idx}_{sample}"]
                            print(lighting_options)
                            options = tasks[str(idx)]
                            options.update(lighting_options)
